...
shared_ptr<BuildingObject> IfcAirTerminalBox::getDeepCopy( BuildingCopyOptions& options )
{
...
void IfcAirTerminalBox::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcAirTerminalBox::toString() const { return L"IfcAirTerminalBox"; }
void IfcAirTerminalBox::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcAirTerminalBox, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
...
void IfcAirTerminalBox::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcFlowController::getAttributes( vec_attributes );
	vec_attributes.emplace_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcAirTerminalBox::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcFlowController::getAttributesInverse( vec_attributes_inverse );
}
void IfcAirTerminalBox::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
