...
shared_ptr<BuildingObject> IfcConstructionEquipmentResource::getDeepCopy( BuildingCopyOptions& options )
{
...
void IfcConstructionEquipmentResource::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcConstructionEquipmentResource::toString() const { return L"IfcConstructionEquipmentResource"; }
void IfcConstructionEquipmentResource::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 11 ){ std::stringstream err; err << "Wrong parameter count for entity IfcConstructionEquipmentResource, expecting 11, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
...
void IfcConstructionEquipmentResource::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcConstructionResource::getAttributes( vec_attributes );
	vec_attributes.emplace_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcConstructionEquipmentResource::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcConstructionResource::getAttributesInverse( vec_attributes_inverse );
}
void IfcConstructionEquipmentResource::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
