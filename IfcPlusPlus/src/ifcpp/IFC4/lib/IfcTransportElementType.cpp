...
shared_ptr<BuildingObject> IfcTransportElementType::getDeepCopy( BuildingCopyOptions& options )
{
...
void IfcTransportElementType::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcTransportElementType, expecting 10, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
...
void IfcTransportElementType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcElementType::getAttributesInverse( vec_attributes_inverse );
}
void IfcTransportElementType::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
