...
shared_ptr<BuildingObject> IfcControllerType::getDeepCopy( BuildingCopyOptions& options )
{
...
void IfcControllerType::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcControllerType::toString() const { return L"IfcControllerType"; }
void IfcControllerType::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcControllerType, expecting 10, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
...
void IfcControllerType::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcDistributionControlElementType::getAttributes( vec_attributes );
	vec_attributes.emplace_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcControllerType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcDistributionControlElementType::getAttributesInverse( vec_attributes_inverse );
}
void IfcControllerType::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
