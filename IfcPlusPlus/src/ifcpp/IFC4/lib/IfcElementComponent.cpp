...
shared_ptr<BuildingObject> IfcElementComponent::getDeepCopy( BuildingCopyOptions& options )
{
...
void IfcElementComponent::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcElementComponent::toString() const { return L"IfcElementComponent"; }
void IfcElementComponent::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 8 ){ std::stringstream err; err << "Wrong parameter count for entity IfcElementComponent, expecting 8, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
...
void IfcElementComponent::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcElement::getAttributes( vec_attributes );
}
void IfcElementComponent::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcElement::getAttributesInverse( vec_attributes_inverse );
}
void IfcElementComponent::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
