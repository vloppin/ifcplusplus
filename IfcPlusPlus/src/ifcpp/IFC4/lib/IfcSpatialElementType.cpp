...
shared_ptr<BuildingObject> IfcSpatialElementType::getDeepCopy( BuildingCopyOptions& options )
{
...
void IfcSpatialElementType::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcSpatialElementType::toString() const { return L"IfcSpatialElementType"; }
void IfcSpatialElementType::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcSpatialElementType, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
...
void IfcSpatialElementType::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcTypeProduct::getAttributes( vec_attributes );
	vec_attributes.emplace_back( std::make_pair( "ElementType", m_ElementType ) );
}
void IfcSpatialElementType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcTypeProduct::getAttributesInverse( vec_attributes_inverse );
}
void IfcSpatialElementType::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
