...
void IfcSpatialStructureElementType::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcSpatialStructureElementType::toString() const { return L"IfcSpatialStructureElementType"; }
void IfcSpatialStructureElementType::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcSpatialStructureElementType, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
...
void IfcSpatialStructureElementType::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcSpatialElementType::getAttributes( vec_attributes );
}
void IfcSpatialStructureElementType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcSpatialElementType::getAttributesInverse( vec_attributes_inverse );
}
void IfcSpatialStructureElementType::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
