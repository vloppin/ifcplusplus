...
shared_ptr<BuildingObject> IfcSlabStandardCase::getDeepCopy( BuildingCopyOptions& options )
{
...
void IfcSlabStandardCase::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcSlabStandardCase::toString() const { return L"IfcSlabStandardCase"; }
void IfcSlabStandardCase::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcSlabStandardCase, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
...
void IfcSlabStandardCase::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcSlab::getAttributes( vec_attributes );
}
void IfcSlabStandardCase::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcSlab::getAttributesInverse( vec_attributes_inverse );
}
void IfcSlabStandardCase::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
