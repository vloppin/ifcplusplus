...
void IfcFeatureElement::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcFeatureElement::toString() const { return L"IfcFeatureElement"; }
void IfcFeatureElement::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 8 ){ std::stringstream err; err << "Wrong parameter count for entity IfcFeatureElement, expecting 8, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
...
void IfcFeatureElement::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcElement::getAttributes( vec_attributes );
}
void IfcFeatureElement::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcElement::getAttributesInverse( vec_attributes_inverse );
}
void IfcFeatureElement::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
