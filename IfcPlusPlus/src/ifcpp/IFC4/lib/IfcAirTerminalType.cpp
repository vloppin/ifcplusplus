...
shared_ptr<BuildingObject> IfcAirTerminalType::getDeepCopy( BuildingCopyOptions& options )
{
...
void IfcAirTerminalType::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcAirTerminalType::toString() const { return L"IfcAirTerminalType"; }
void IfcAirTerminalType::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcAirTerminalType, expecting 10, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
...
void IfcAirTerminalType::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcFlowTerminalType::getAttributes( vec_attributes );
	vec_attributes.emplace_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcAirTerminalType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcFlowTerminalType::getAttributesInverse( vec_attributes_inverse );
}
void IfcAirTerminalType::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
