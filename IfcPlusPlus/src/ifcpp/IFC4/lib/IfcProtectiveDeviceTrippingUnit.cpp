...
shared_ptr<BuildingObject> IfcProtectiveDeviceTrippingUnit::getDeepCopy( BuildingCopyOptions& options )
{
...
void IfcProtectiveDeviceTrippingUnit::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcProtectiveDeviceTrippingUnit::toString() const { return L"IfcProtectiveDeviceTrippingUnit"; }
void IfcProtectiveDeviceTrippingUnit::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcProtectiveDeviceTrippingUnit, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
...
void IfcProtectiveDeviceTrippingUnit::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcDistributionControlElement::getAttributes( vec_attributes );
	vec_attributes.emplace_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcProtectiveDeviceTrippingUnit::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcDistributionControlElement::getAttributesInverse( vec_attributes_inverse );
}
void IfcProtectiveDeviceTrippingUnit::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
