...
void IfcStair::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcStair, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
...
void IfcStair::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcBuildingElement::getAttributes( vec_attributes );
	vec_attributes.emplace_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcStair::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcBuildingElement::getAttributesInverse( vec_attributes_inverse );
}
void IfcStair::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
