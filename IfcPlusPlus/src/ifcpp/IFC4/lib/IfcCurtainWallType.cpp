...
shared_ptr<BuildingObject> IfcCurtainWallType::getDeepCopy( BuildingCopyOptions& options )
{
...
void IfcCurtainWallType::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcCurtainWallType::toString() const { return L"IfcCurtainWallType"; }
void IfcCurtainWallType::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcCurtainWallType, expecting 10, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
...
void IfcCurtainWallType::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcBuildingElementType::getAttributes( vec_attributes );
	vec_attributes.emplace_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcCurtainWallType::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcBuildingElementType::getAttributesInverse( vec_attributes_inverse );
}
void IfcCurtainWallType::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
