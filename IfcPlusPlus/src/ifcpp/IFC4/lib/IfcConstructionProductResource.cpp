...
shared_ptr<BuildingObject> IfcConstructionProductResource::getDeepCopy( BuildingCopyOptions& options )
{
...
void IfcConstructionProductResource::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcConstructionProductResource::toString() const { return L"IfcConstructionProductResource"; }
void IfcConstructionProductResource::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 11 ){ std::stringstream err; err << "Wrong parameter count for entity IfcConstructionProductResource, expecting 11, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
...
void IfcConstructionProductResource::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcConstructionResource::getAttributes( vec_attributes );
	vec_attributes.emplace_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcConstructionProductResource::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcConstructionResource::getAttributesInverse( vec_attributes_inverse );
}
void IfcConstructionProductResource::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
