...
shared_ptr<BuildingObject> IfcCableSegment::getDeepCopy( BuildingCopyOptions& options )
{
...
void IfcCableSegment::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcCableSegment::toString() const { return L"IfcCableSegment"; }
void IfcCableSegment::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcCableSegment, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
...
void IfcCableSegment::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcFlowSegment::getAttributesInverse( vec_attributes_inverse );
}
void IfcCableSegment::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
