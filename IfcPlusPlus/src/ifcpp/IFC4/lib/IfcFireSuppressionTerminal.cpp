...
void IfcFireSuppressionTerminal::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	...
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcFireSuppressionTerminal, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
...
void IfcFireSuppressionTerminal::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
