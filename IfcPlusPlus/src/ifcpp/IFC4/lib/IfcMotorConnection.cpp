...
shared_ptr<BuildingObject> IfcMotorConnection::getDeepCopy( BuildingCopyOptions& options )
{
...
void IfcMotorConnection::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_entity_id; }
const std::wstring IfcMotorConnection::toString() const { return L"IfcMotorConnection"; }
void IfcMotorConnection::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<BuildingEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcMotorConnection, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw BuildingException( err.str().c_str() ); }
...
void IfcMotorConnection::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IfcEnergyConversionDevice::getAttributes( vec_attributes );
	vec_attributes.emplace_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IfcMotorConnection::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IfcEnergyConversionDevice::getAttributesInverse( vec_attributes_inverse );
}
void IfcMotorConnection::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
